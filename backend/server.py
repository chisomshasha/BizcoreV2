    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.method not in ["GET", "OPTIONS", "HEAD"]:
            ...
            allowed_origins = [
                ...
                "http://127.0.0.1",
                "http://127.0.0.1:3000",
                None,   # Allow requests with no Origin header (mobile app, Railway proxy)
            ]
            if origin and origin not in allowed_origins:
                # Safety net: allow any *.railway.app subdomain
                if not origin.endswith(".railway.app"):
                    return JSONResponse(
                        status_code=403,
...
async def forgot_password(request: Request, body: ForgotPasswordRequest):
    """
    Generate a 6-digit OTP reset code valid for 15 minutes.
    The code is stored hashed in the DB. The plaintext code is returned in
    the response so the caller (admin/support) can relay it to the user via
    any out-of-band channel (e.g. SMS, WhatsApp).  When an SMTP_URL env var
    is present the endpoint will also send an email automatically.
    Rate limited: 5/minute.
    """
    sanitized_email = sanitize_string(body.email.strip().lower())
    user_doc = await db.users.find_one({"email": sanitized_email}, {"_id": 0})
    ...
    # Upsert — only one active reset per user
    await db.password_reset_tokens.update_one(
        {"user_id": user_doc["user_id"]},
        {"$set": {"otp_hash": otp_hash, "expires_at": expires_at, "used": False}},
        upsert=True,
    )

    # Optional: send email if SMTP_URL is configured
    ...
    if smtp_url:
        try:
            ...
            msg = MIMEText(
                f"Your BizCore password reset code is: {otp}\n\nThis code expires in 15 minutes.",
                "plain",
            )
            msg["Subject"] = "BizCore – Password Reset Code"
            msg["From"] = parsed.username or "noreply@bizcore.app"
            msg["To"] = sanitized_email
            context = ssl.create_default_context()
            with smtplib.SMTP_SSL(parsed.hostname, parsed.port or 465, context=context) as s:
                s.login(parsed.username, parsed.password)
                s.sendmail(msg["From"], [sanitized_email], msg.as_string())
        except Exception as e:
            logging.warning(f"Failed to send reset email: {e}")

    return {
        "message": "Reset code generated. Share it with the user via a secure channel.",
        "otp": otp,          # Show in app so admin/support can relay it
...
async def reset_password(request: Request, body: ResetPasswordRequest):
    ...
    otp = body.token.strip()

    # Find a non-expired, unused token that matches
    ...
    for doc in token_docs:
        try:
            if bcrypt.checkpw(otp.encode(), doc["otp_hash"].encode()):
                matched_doc = doc
                break
        except Exception:
            continue
...
@limiter.limit("20/minute")
async def delete_user(request: Request, user_id: str, current_user: User = Depends(get_current_user)):
    """
    ...
    """
    if current_user.role != UserRole.SUPER_ADMIN:
        raise HTTPException(status_code=403, detail="Only SuperAdmin can delete user accounts")
    ...
    if target.get("role") == UserRole.SUPER_ADMIN.value:
        raise HTTPException(status_code=403, detail="Cannot delete another Super Admin account")
    ...
    await create_audit_log(
        current_user.user_id, "delete", "user", user_id,
        old_value={"name": target.get("name"), "email": target.get("email"), "role": target.get("role")},
        request=request,
    )
    return {"message": f"User account for {target.get('name')} has been deleted"}
...
async def create_product(product: ProductCreate, user: User = Depends(get_current_user)):
    ...
    # Seed zero-quantity inventory_stock for every active warehouse so
    # dashboard metrics compute immediately after a product is created.
    warehouses = await db.warehouses.find({"is_active": True}, {"_id": 0, "warehouse_id": 1}).to_list(500)
    for wh in warehouses:
        exists = await db.inventory_stock.find_one({
        ...
        if not exists:
            ...
            await db.inventory_stock.insert_one(seed.model_dump())
...
# ========================
# MIDDLEWARE SETUP (RAILWAY DEPLOYMENT)
# Note: Starlette middleware runs in LIFO order.
# Execution order on REQUEST:  RequestValidation → SecurityHeaders → TrustedHost → CORS
# Execution order on RESPONSE: CORS → TrustedHost → SecurityHeaders → RequestValidation
# ========================

# TrustedHostMiddleware — wildcard safe behind Railway's proxy
app.add_middleware(
    TrustedHostMiddleware,
    allowed_hosts=[
        "*",                              # Railway proxy rewrites Host header — accept all
        "*.up.railway.app",
        "localhost",
        "127.0.0.1",
        "0.0.0.0",
...
# CORS — Railway backend URL + mobile app scheme
app.add_middleware(
    CORSMiddleware,
    allow_credentials=True,
    allow_origins=[
        "https://bizcorev2-production.up.railway.app",
        "bizcore://",                     # React Native deep link scheme
        "http://localhost:3000",
        "http://127.0.0.1:3000",
