...
export default function InventoryScreen() {
  ...
  const handleAddProduct = async () => {
    try {
      ...
      // If a warehouse + initial quantity were provided, add stock immediately
      const qty = parseFloat(formData.initial_quantity);
      if (formData.initial_warehouse_id && qty > 0) {
        try {
          const productId = (created as any)?.product_id;
          if (productId) {
          ...
          // Product created but stock failed — warn without blocking
          Alert.alert(
            'Product Created',
            'Product was created, but initial stock could not be added: ' +
              (stockErr.response?.data?.detail || 'Unknown error'),
          );
              ...
              <Card key={stock.stock_id} style={styles.stockCard}>
                  ...
                  <View style={styles.stockQuantity}>
                    <Text
                      style={[
                        ...
                        {
                          color:
                            stock.quantity > 0 && stock.quantity <= (stock.reorder_level || 10)
                              ? Colors.danger
                              : Colors.success,
                        },
                      ]}
                ...
                <View style={styles.stockFooter}>
                  ...
                  <Text style={styles.stockValue}>
                    Value: {formatCurrency((stock.quantity || 0) * (stock.cost_price || 0))}
                  </Text>
                </View>
              </Card>
            ))
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { paddingBottom: Math.max(insets.bottom, 20) }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <Text style={styles.inputHint}>Select a warehouse and enter opening quantity to add stock immediately.</Text>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setFormData({
                        ...formData,
                        initial_warehouse_id:
                          formData.initial_warehouse_id === wh.warehouse_id ? '' : wh.warehouse_id,
                      })
                    }
                  >
                    <Ionicons
                      ...
                      size={14}
                      color={formData.initial_warehouse_id === wh.warehouse_id ? '#FFFFFF' : Colors.textSecondary}
                    />
                    <Text
                      style={[
                        styles.warehouseOptionText,
                        formData.initial_warehouse_id === wh.warehouse_id && styles.warehouseOptionTextActive,
                      ]}
                    >
                      {wh.name}
                    </Text>
