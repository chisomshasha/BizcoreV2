...
import clientConfig, { isFeatureEnabled, formatCurrency } from '../../src/config/clientConfig';
...
export default function MoreScreen() {
  ...
  const [warehouseList, setWarehouseList] = useState<{warehouse_id: string; name: string}[]>([]);
  ...
  const loadWarehouseList = async () => {
    try {
      ...
      setWarehouseList(res.data.map((w: any) => ({ warehouse_id: w.warehouse_id, name: w.name })));
    } catch {}
  };
  ...
  const submitAddUser = async () => {
    const { name, email, phone, username, password, role, warehouse_id } = newUser;
    ...
    try {
      ...
      loadUsers();
      Alert.alert('Success', `Account created for ${name}. They can now log in with their username and password.`);
    } catch (e: any) {
  ...
  const handleDeleteUser = (u: any) => {
    ...
    Alert.alert(
      'Delete User',
      `Permanently delete the account for "${u.name}"? This cannot be undone.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Deleted', `${u.name}'s account has been removed.`);
              loadUsers();
            } catch (e: any) {
              Alert.alert('Error', e.response?.data?.detail || 'Failed to delete user.');
            }
          },
        },
      ]
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
          ...
          <View style={[styles.modalContent, { paddingBottom: Math.max(insets.bottom, 20) }]}>
            <View style={styles.modalHeader}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                {isSuperAdmin && (
                  <TouchableOpacity
                    onPress={() => { setShowUsersModal(false); setTimeout(openAddUser, 300); }}
                    style={styles.addUserIconBtn}
                  >
              ...
              {users.length === 0 ? (
                ...
                users.map((u) => (
                  <View key={u.user_id} style={styles.userItem}>
                    ...
                    {isSuperAdmin && u.user_id !== user?.user_id && (
                      <TouchableOpacity
      ...
      {/* Add User Modal */}
      <Modal visible={showAddUserModal} transparent animationType="slide" onRequestClose={() => setShowAddUserModal(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
          <View style={[styles.modalContent, { paddingBottom: Math.max(insets.bottom, 20) }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              {([
                { key: 'name',     label: 'Full Name',              placeholder: 'e.g. Amaka Obi',         secure: false },
                { key: 'email',    label: 'Email',                   placeholder: 'user@company.com',       secure: false },
                { key: 'phone',    label: 'Phone',                   placeholder: '+234 800 000 0000',      secure: false },
                { key: 'username', label: 'Username',                placeholder: 'lowercase, no spaces',   secure: false },
                { key: 'password', label: 'Password (min 8 chars)',  placeholder: '••••••••',               secure: true  },
              ] as {key: string; label: string; placeholder: string; secure: boolean}[]).map(f => (
                <View key={f.key} style={styles.addUserField}>
                  <Text style={styles.addUserLabel}>{f.label}</Text>
                  <TextInput
              ...
              <View style={styles.addUserField}>
                <Text style={styles.addUserLabel}>Role</Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 6 }}>
                  {['viewer','sales_clerk','purchase_clerk','sales_rep','sales_executive',
                    'accountant','warehouse_manager','manager','general_manager'].map(r => (
                    <TouchableOpacity
                      key={r}
              ...
              {warehouseList.length > 0 && (
                <View style={styles.addUserField}>
                  <Text style={styles.addUserLabel}>Warehouse (optional)</Text>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 6 }}>
                    <TouchableOpacity
              ...
              <TouchableOpacity
                style={[styles.createUserBtn, creatingUser && { opacity: 0.6 }]}
                onPress={submitAddUser}
                disabled={creatingUser}
              >
                {creatingUser
                  ? <ActivityIndicator size="small" color="#fff" />
                  : <Text style={styles.createUserBtnText}>Create User Account</Text>
                }
              </TouchableOpacity>
            </ScrollView>
          </View>
                  ...
                  <View style={styles.reportSummary}>
                    <View style={styles.reportSummaryItem}>
                      <Text style={styles.reportSummaryValue}>
                        {formatCurrency(stockReport.reduce((sum, item) => sum + (item.value || 0), 0))}
                      </Text>
